<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="获奖证书" prop="huojiangzhengshu">
          <file-upload
          tip="点击上传获奖证书"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.huojiangzhengshu?ruleForm.huojiangzhengshu:''"
          @change="huojiangzhengshuUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.huojiangzhengshu" label="获奖证书" prop="huojiangzhengshu">
            <el-button type="text" size="small" @click="download(ruleForm.huojiangzhengshu)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiangxixinxi = this.ruleForm.xiangxixinxi.replace(reg,'../../../ssm9m0be/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiaoshirongyu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiaoshirongyuCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    huojiangzhengshuUploadChange(fileUrls) {
	this.ruleForm.huojiangzhengshu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
